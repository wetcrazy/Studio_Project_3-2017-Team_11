...
void SceneKinematics::Update(double dt)
{
	...
	else if(bLButtonState && !Application::IsMousePressed(0))
	{
		...
		for (std::vector<GameObject*>::iterator it = m_goList.begin(); it != m_goList.end(); ++it)
		{
			...
			if (!go->active)
			{
				...
				go->type = GameObject::GO_BALL;
				go->pos.Set(x / w * m_worldWidth, (h - y) / h * m_worldHeight, 0);
				go->vel.Set(newVel.x, newVel.y, 0);
		...
		m_timeEstimated2 = (sqrt((m_heightEstimated * 2) / -m_gravity.y)) + m_timeEstimated1;
		m_timeTaken2 = 0;
		//v = u + a * t
		//t = (v - u ) / a
	...
	int w = Application::GetWindowWidth();
	int h = Application::GetWindowHeight();
	newVel = m_ghost->pos - Vector3(x / w * m_worldWidth, (h - y) / h * m_worldHeight, 0);
	...
	for(std::vector<GameObject *>::iterator it = m_goList.begin(); it != m_goList.end(); ++it)
	{
		...
		if(go->active)
		{
			...
			for (std::vector<GameObject*>::iterator bit = m_goList.begin(); bit != m_goList.end(); ++bit)
			{
				GameObject *gob = (GameObject *)*bit;
				if (go != gob)
				{
					if (gob->active && gob->type == GameObject::GO_CUBE)
						if ((go->pos - gob->pos).Length() < (go->scale.x + sqrt((gob->scale.x * gob->scale.x) + (gob->scale.y * gob->scale.y))))
						{
							go->active = false;
							gob->active = false;
							break;
...
void SceneKinematics::Render()
{
	...
	{
		ss.str("");
		ss << "Est Horizontal dist: " << m_timeEstimated2 * m_timeGO->vel.x;
		RenderTextOnScreen(meshList[GEO_TEXT], ss.str(), Color(0, 1, 0), 3, 0, 33);

		ss.str("");
		ss << "Horizontal distance: " << m_timeTaken2 * m_timeGO->vel.x;
