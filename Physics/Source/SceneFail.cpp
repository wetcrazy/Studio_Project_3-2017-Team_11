...
void SceneFail::CreateStuff()
{
	...
	FailMenu->type = GameObject::GO_FAILMENU;	// Background
	FailMenu->active = true;
	FailMenu->pos.Set(-10, -10, 1);
	FailMenu->scale.Set(1, 1, 1);
...
bool SceneFail::CheckCollision(GameObject *go1, GameObject *go2, float dt)
{
	switch (go2->type)
	{
...
float SceneFail::CheckCollision2(GameObject *go1, GameObject *go2)
{
...
void SceneFail::CollisionResponse(GameObject * go1, GameObject * go2)
{
	switch (go2->type)
	{
	case GameObject::GO_BALL:
	...
	//UP, DOWN and ENTER controls
	{
		...
		if ((Application::IsKeyPressed(VK_LEFT)) && pressDelay >= cooldownPressed)
		{
			if (selectOptions == EXIT_Fail)
				selectOptions = DELETE_Fail;

			else if (selectOptions == DELETE_Fail)
				selectOptions = EXIT_Fail;
